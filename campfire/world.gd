...
func worldgen():
			...
			if relfloor < 4:
				...
				for x in range(worldsize):
					for y in range(worldsize):
						...
						var relative = noisevalue - (1 - 2 ** -relfloor)
						if relative > 0:
							carpetmap[Vector2(x, y)] = true
							var strength = relative / 2 ** -relfloor
							if strength > 0.25:
								var size = strength / 2 ** (relfloor / 2.)
								for i in range(8):
				...
				print(topheight + {mossmesh: 0, glowmesh: -totalheight}[texture])
	...
	for i in range(len(vinetransforms)):
		$GrassMultiMesh.multimesh.set_instance_transform(i, vinetransforms[i])
	Global.map = map
	print("checkpoint 10")
	...
	if playerfloor != lastfloor:
		...
		for floori in range(numfloors):
			if abs(floori - playerfloor - 1) <= 1:
				...
				if loaded[floori]:
					unloadfloor(floori)
		var lightfraction = (1 - (playerfloor + 1) / numfloors) ** 3
